function ibs_util_19(){return"AIzaSyBwcmfwl7W1aMyo9wnXwmASRfZ0sOhGhRc"}function CalendarObj(a,d,h){this.init(d,h)}
...
a("#list-display-"+c.id).parent().css("display","none"):a("#list-display-"+c.id).prop("checked","show"===d.event_list);this.calendar.fullCalendar(this.fullcalendar_options);for(var f in this.options.feeds)""!==this.options.feeds[f].url&&this.options.feeds[f].enabled&&this.calendar.fullCalendar("addEventSource",{googleCalendarApiKey:function(){return"string"===typeof c.options.feeds[f].key&&""!==c.options.feeds[f].key?c.options.feeds[f].key:ibs_util_19()},textColor:this.options.feeds[f].textColor,
backgroundColor:this.options.feeds[f].backgroundColor,url:this.options.feeds[f].url});d.ibsEvents&&this.calendar.fullCalendar("addEventSource",{events:function(a,d,e,f){e=[];for(var l in c.ibs_events){var g=c.ibs_events[l];if(0==g.recurr){var m,n,h,r;m=a.unix();n=d.unix();h=moment(g.start).unix();r=moment(g.end).unix();(h>=m&&r<=n||r>m&&r<n||h>=m&&h<=n||m>=h&&n<=r)&&e.push(g)}else{m=[];if(g.exceptions){m=g.exceptions.split(",");for(var p in m)m[p]=moment(m[p]).startOf("day")}n=(new RRule(RRule.parseString(g.repeat))).between(a.toDate(),
d.toDate());for(p in n)n[p]=moment(n[p]).startOf("day");h=moment(g.end).diff(moment(g.start),"seconds");r=moment(g.start).unix()-moment(g.start).startOf("day").unix();for(p in n){var q;a:{q=p;var t=void 0;for(t in m)if(0===m[t].diff(n[q])){q=!0;break a}q=!1}q||(q=n[p].startOf("day"),q={start:q.add(r,"seconds").format(),end:q.add(h,"seconds").format(),id:g.id,title:g.title,allDay:g.allDay,color:g.color,textColor:g.textColor,description:g.description,url:g.url,repeat:g.repeat,exceptions:g.exceptions},
e.push(q))}}for(p in e)e[p].textColor="#ffffff",e[p].editable=!1}f(e)}});"none"!==d.event_list&&("show"===d.event_list&&a("#event-list-"+c.id).show(),a("#list-display-"+c.id).click(function(b){a("#list-display-"+c.id).is(":checked")?(a("#fullcalendar-"+c.id).fullCalendar("rerenderEvents"),a("#event-list-"+c.id).show()):a("#event-list-"+c.id).hide()}))};d.ibsEvents?a.get(c.options.ajaxUrl,{action:"ibs_calendar_get_events",cache:!1,dataType:"json"}).then(function(a){if(""!==a){a=decodeURIComponent(a);
c.ibs_events=JSON.parse(a);for(var b in c.ibs_events)c.ibs_events[b].title=jQuery("<div>").html(c.ibs_events[b].title).text(),c.ibs_events[b].description=jQuery("<div>").html(c.ibs_events[b].description).text(),c.ibs_events[b].editable=!1,c.ibs_events[b].start=moment.unix(parseInt(c.ibs_events[b].start)).format(),c.ibs_events[b].end=moment.unix(parseInt(c.ibs_events[b].end)).format();console.log("IBS Events loaded.")}else c.ibs_events=[];c.renderCalendar()},function(){console.log("Get IBS Events failed.")}):
c.renderCalendar()}})(jQuery);
